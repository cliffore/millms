...
thisProcess = "calculate-distances.py"
...
output_csv = ex_folder + "/distances_with_groundtruth_fve.csv"


# Load all .npy files and map concept name to tensor
concept_tensors = {}
for fname in os.listdir(data_folder):
    if fname.endswith(".npy") and  "weighted_avg" in fname and "fve" in fname:
...
output_csv = ex_folder + "/distances_with_groundtruth_cve.csv"


# Load all .npy files and map concept name to tensor
concept_tensors = {}
for fname in os.listdir(data_folder):
    if fname.endswith(".npy") and  "weighted_avg" in fname and "cve" in fname:
        concept = fname.replace(".npy", "")  # e.g., "edas-Author"
        tensor = np.load(os.path.join(data_folder, fname))
        concept_tensors[concept] = tensor
...
for (concept1, tensor1), (concept2, tensor2) in combinations(concept_tensors.items(), 2):
    cnt = cnt + 1
    if cnt % 100 == 0:
        print("English and Chinese average: " + str(cnt))
    if tensor1.shape != tensor2.shape:
        target_len = max(len(t) for t in concept_tensors.values())
        tensor1 = resize_vector(tensor1, target_len)
        tensor2 = resize_vector(tensor2, target_len)
...
print(f"Saved English and Chinese average distances and averages ground truth to: {output_csv}")
...
output_csv = ex_folder + "/distances_with_groundtruth_eo.csv"

# Load all .npy files and map concept name to tensor
concept_tensors_2 = {}
for fname in os.listdir(data_folder):
    if fname.endswith(".npy") and '-fr--' not in fname and '-zh--' not in fname and  "weighted_avg" not in fname:
        concept = fname.replace(".npy", "")  # e.g., "edas-Author"
        tensor = np.load(os.path.join(data_folder, fname))
...
output_csv = ex_folder + "/distances_with_groundtruth_fo.csv"
...
output_csv = ex_folder + "/distances_with_groundtruth_co.csv"

# Load all .npy files and map concept name to tensor
concept_tensors_2 = {}
for fname in os.listdir(data_folder):
    if fname.endswith(".npy") and '-zh--' in fname and  "weighted_avg" not in fname:
        concept = fname.replace(".npy", "")  # e.g., "edas-Author"
        tensor = np.load(os.path.join(data_folder, fname))
        concept_tensors_2[concept] = tensor
...
for (concept1, tensor1), (concept2, tensor2) in combinations(concept_tensors_2.items(), 2):
    cnt = cnt + 1
    if cnt % 100 == 0:
        print("Chinese only distances: " + str(cnt))
    if tensor1.shape != tensor2.shape:
        target_len = max(len(t) for t in concept_tensors_2.values())
        tensor1 = resize_vector(tensor1, target_len)
        tensor2 = resize_vector(tensor2, target_len)
...
print(f"Saved Chinese distances and ground truth to: {output_csv}")
